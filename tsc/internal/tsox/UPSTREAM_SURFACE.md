...
| `internal/ast` — `Node`, `SourceFile`, `Symbol`, `ModifierList`; `Node.ForEachChild`, `Node.Text`, `Node.Name`, `Node.Pos`; the guarded `As*` accessors and fields for source files, blocks, variable/parameter/function declarations, expression/return/if/while/for statements, identifiers/literals, binary/prefix/postfix/call/property-access/template/arrow/parenthesized expressions; the corresponding `Kind*` constants; `NodeFlagsBlockScoped`, `NodeFlagsLet`, `NodeFlagsConst`; `SymbolFlagsValue` | Checked syntax shape, lexical symbol identity, declaration mutability, literal/operator data, and source byte ranges | Build the closed graph allow-list and preserve binding identity without retaining AST or symbol pointers; phase-one tests also pin the recursive traversal pattern |
...
| `internal/core` — `CompilerOptions`, `TSTrue`, `UTF16Offset` | Strict/single-threaded configuration and diagnostic column units | Keep one checker identity domain and report TypeScript-compatible source columns |
